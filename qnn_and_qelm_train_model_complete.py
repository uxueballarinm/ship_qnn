...
class FrozenQNNWrapper:
    ...
    def __init__(self, original_model, initial_params):
        ...
        print(f"[FrozenQNN] Frozen: {len(self.q_indices)} | Active: {self.total_params}")
...
def run(args):
    ...
    if found_path is not None and getattr(args, 'check_existing', True):
        ...
        save_experiment_results(args, train_res, 
            ...
            freeze = saved_data.get('freeze_qnn', False))
        return 
    ...
    if args.model == 'multihead':
        ...
        for i, head_cfg in enumerate(args.heads_config):
            ...
            head_model = WindowEncodingQNN(estimator_qnn, (0, args.horizon, num_outputs), seed=seed)
        ...
        backend = AerSimulator(seed_simulator=seed)
        estimator_qnn = EstimatorQNN(circuit=qc, input_params=in_p, weight_params=w_p, observables=obsvs, estimator=estimator,  pass_manager=generate_preset_pass_manager(backend=backend, optimization_level=1, seed_transpiler=seed))
        model = WindowEncodingQNN(estimator_qnn, y_train.shape, seed=seed)
    ...
    if args.optimizer.lower() == 'ridge':
        ...
        results = train_qelm_ridge(args, model, x_train, y_train, x_val, y_val, alpha=args.learning_rate)
        model_to_train,selected_weights_type = model, "Ridge Optimal" # No actual training loop, so we just use the original model and label the selection type accordingly
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--reorder', type=str2bool, default=False, choices=[True, False])
